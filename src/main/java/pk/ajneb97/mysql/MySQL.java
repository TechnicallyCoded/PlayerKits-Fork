...
//	}
    ...
    public static void getJugadorByUUID(final Player jugador, final String uuid, final PlayerKits plugin, final MySQLJugadorCallback callback) {
        Runnable runnable = () -> {
            ...
            if (kitsJugador.size() >= 1) {
                ...
                plugin.getFoliaLib().getImpl().runAtEntity(jugador, (task) -> callback.alTerminar(j));
            } else {
                plugin.getFoliaLib().getImpl().runAtEntity(jugador, (task) -> callback.alTerminar(null));
            }
        };

        plugin.getFoliaLib().getImpl().runAsync((task) -> runnable.run());
    }

    public static void getJugadorByName(final Player jugador, final String nombre, final PlayerKits plugin, final MySQLJugadorCallback callback) {
        plugin.getFoliaLib().getImpl().runAsync((task) -> {
            ...
            if (kitsJugador.size() >= 1) {
                final JugadorDatos j = new JugadorDatos(nombre, uuid, kitsJugador);
                plugin.getFoliaLib().getImpl().runAtEntity(jugador, (task2) -> callback.alTerminar(j));
            } else {
                plugin.getFoliaLib().getImpl().runAtEntity(jugador, (task2) -> callback.alTerminar(null));
            }
        });
    }
